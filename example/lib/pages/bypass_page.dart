...
class _BypassPageState extends State<BypassPage> {
  ...
  void _onSuccess(CfBypassResult result) {
    ...
    if (!mounted) return;
    setState(() { _state = _BypassState.success; _result = result; });
  ...
  void _onFailure(CfBypassResult result) {
    ...
    if (!mounted) return;
    setState(() { _state = _BypassState.failed; _result = result; });
  }
  ...
  void _onPageStartedLoading(String? url) {
    ...
    if (mounted && _state != _BypassState.success && _state != _BypassState.failed) {
  ...
  void _onError(Object error) {
    _log('WebView error: $error', icon: Icons.warning_amber_rounded, color: _red);
  }
  ...
  void _retry() {
    if (!mounted) return;
    setState(() { _loopDetected = false; _state = _BypassState.loading; });
    _log('Retrying — cleared CF cookies', icon: Icons.refresh_rounded, color: _amber);
    _controller.retry();
  }
    ...
    return Positioned(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border(
            bottom: BorderSide(color: (ok ? _green : _red).withValues(alpha: 0.3)),
          ),
        ),
        child: Row(
          children: [
                          ...
                          child: Row(
                            ...
                            children: [
                              Text(
                                _ts(e.time),
                                style: const TextStyle(
                                  color: _muted, fontSize: 10, fontFamily: 'monospace',
                                ),
                              ),
                              const SizedBox(width: 8),
